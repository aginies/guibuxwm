...
- **Topbar styling:** a faint 1px highlight line along the top edge (raised look against the wallpaper) and a 1px `color_border` line along the bottom edge; groups (workspaces / window pills / indicator items / clock) are divided by 3px vertical separators with a vertical alpha gradient (fading out at both ends) so they read as soft dividers; every item keeps a clear margin from its separators
...
- **Drag window to workspace:** press a window pill and drag to a workspace number to move the window to that workspace; the workspace cell under the cursor is highlighted while dragging; a short click (no drag) still focuses/switches as before
