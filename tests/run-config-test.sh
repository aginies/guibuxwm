...
# WLR_RENDERER=vulkan (env) must win over the config's `renderer = pixman`
grep -q "renderer: vulkan" "$log" || { echo "config-test: FAIL env WLR_RENDERER did not win over config"; ok=0; }
