...
# case_run <desc> <GUIBUX_OUTPUTS value, "-" = unset> <hook mode> <use client 0/1>
case_run() {
  desc=$1
  outputs_val=$2
  hook=$3
  use_client=$4
  log=$(mktemp)
  envs=(GUIBUX_TEST_EXTRA_OUTPUTS=1 GUIBUX_TEST_OUTPUTS=$hook GUIBUX_TERM=true WLR_RENDERER=gles2)
  if [ "$outputs_val" != "-" ]; then
